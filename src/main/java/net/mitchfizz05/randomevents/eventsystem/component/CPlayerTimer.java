 ...
 */
public class CPlayerTimer implements IComponent, IUsesNBT
{
    ...
    public CPlayerTimer(int minWaitTime, int maxWaitTime)
    {
    ...
    public CWorldTimer getTimer(UUID uuid)
    {
        if (!playerTimers.containsKey(uuid)) {
            playerTimers.put(uuid, new CWorldTimer(minWaitTime, maxWaitTime));
        }
