...
public class CustomCompass
{
    public static void init(FMLInitializationEvent event)
    {
        Items.COMPASS.addPropertyOverride(new ResourceLocation("angle"), new IItemPropertyGetter()
        {
            ...
            {
                BlockPos compassTarget = world.getSpawnPoint();

                if (entity instanceof EntityPlayer)
                {
                    // todo: for some reason bedLocation just returns the world spawn - method probably doesn't work client-side?
                    EntityPlayer player = (EntityPlayer) entity;
                    BlockPos bedLocation = player.getBedLocation(0);
                    if (bedLocation != null) compassTarget = bedLocation;
                }

                return Math.atan2((double)compassTarget.getZ() - entity.posZ, (double)compassTarget.getX() - entity.posX);
            }
        });
    }
}
